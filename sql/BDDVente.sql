...
BEGIN
    ...
    BEGIN
        IF (v_Cat = 50 AND v_OQ > 100) THEN
            DELETEligne@site1_link(:OLD.idlignecommande);
        ELSIF (v_Cat = 35 AND v_OQ > 50) THEN
...
FROM BDDVente.clients c, BDDVente.commandes cm
...
CREATE INDEX idx_cmd_date    ON BDDVente.commandes (EXTRACT(YEAR FROM datecommande));
CREATE INDEX idx_cmd_client  ON BDDVente.commandes (idclient);
...
GRANT SELECT_CATALOG_ROLE TO BDDVENTE;
GRANT SELECT ANY DICTIONARY TO BDDVENTE;
...
WHERE DateCommande BETWEEN TO_DATE('01/01/26', 'DD/MM/YYYY') 
                       AND TO_DATE('31/12/26', 'DD/MM/YYYY');
